...
namespace PvZ.Buttons
{
    class ExitGameButton : Buttons
    {
        ...
        public ExitGameButton(Game1 game, bool nToMenuButton)
            : base(game)
        {
            Size = new Vector2(Game1.graphics.PreferredBackBufferWidth / 8, Game1.graphics.PreferredBackBufferHeight / 30);
        ...
        protected override void PositiveButtonEvent()
        {
            if (ToMenuButton)
            {
                ...
                PressedStatus = true;
                MediaPlayer.Play(Game1.ThisGame.MainMenuMusic);
            }
            else
            {
                switch (Game1.ThisGame.GameStateVar)
                {

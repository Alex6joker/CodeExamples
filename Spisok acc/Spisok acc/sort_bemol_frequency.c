...
void sort_bemol_frequency()
{
	...
	max = INT_MIN;
	min = INT_MAX;
	tmp = chtenie_s_file_bemol(k); // Результат функции - голова последней строки того, что можно было считать из файла
	if (tmp==error) 
	{
	...
	if(i==1)
	{
		...
		while(tmp1!=NULL)
		{
			while(tmp2!=NULL)
			{
				if(tmp2->frequency >= tmp->frequency)
				{
					max = tmp2->frequency;
					strcpy(sort_str,tmp2->name);
					strcpy(tmp2->name,tmp->name);

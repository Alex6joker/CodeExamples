...
{
    class TableOfSTART : GPSSTable
    {
        static TableOfSTART InstanseOfTable;
        ...
        public static GPSSTable getInstanse()
        {
            if (InstanseOfTable == null)
                InstanseOfTable = new TableOfSTART();
        ...
        {
            // Блок START может прислать только те имена переменных, которые были ранее определены в EQU
            ...
            BlocksList.Length, new String(new Char[] { '0' }), this.GetType(), Convert.ToInt32(Record[Record.Length - 1]));
